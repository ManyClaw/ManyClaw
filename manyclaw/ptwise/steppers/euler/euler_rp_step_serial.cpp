...
/**
Implements the serial stepping of the euler rieman problem through a structured
2D grid.
 ...
 */
void euler_rp_step_serial(const real* q, const real* aux,
                              ...
                              real* wave, real* wave_speeds)
{
  ...
  for(row = num_ghost; row <= ny + num_ghost; ++row)
  {
    for(col = num_ghost; col <= nx + num_ghost; ++col)
    {
      ...
      euler_rp(q + idx_left*num_states, q + idx_right*num_states,
  ...
  for(col = num_ghost; col <= nx + num_ghost; ++col)
  {
    for(row = num_ghost; row <= ny + num_ghost; ++row)
    {
      ...
      euler_rp(q + idx_up*num_states, q + idx_down*num_states,
                   aux, aux, &euler_rp_aux_global,
                   amdq + idx_out*num_states, apdq + idx_out*num_states,
                   wave + num_waves*num_states*idx_out, wave_speeds + num_waves*idx_out);
    }
