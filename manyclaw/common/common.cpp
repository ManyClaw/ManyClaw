                      ...
                      rp_step_t rp_stepper_1, rp_step_t rp_stepper_2)
{
  ...
  Solver solver_1(solution, params.num_ghost, params.num_waves);
  Solver solver_2(solution, params.num_ghost, params.num_waves);
...
double benchmark_stepper(int nx, int ny, rp_grid_params params, rp_step_t rp_stepper)
{
  ...
  state.randomize();
  Solution solution(grid, state);
  Solver solver(solution, params.num_ghost, params.num_waves);
                        ...
                        rp_step_t rp_stepper, updater_t updater)
{
  ...
  updater(&state.q[0], &state.aux[0], grid.num_cells[0], grid.num_cells[1], 
          &solver.amdq[0],  &solver.apdq[0],  &solver.waves[0], 
          &solver.wave_speeds[0], params.num_ghost, params.num_eqn, 1.0);
