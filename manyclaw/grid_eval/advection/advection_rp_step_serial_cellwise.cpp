...
void advection_rp_step_serial_cellwise( const real* q,  const real* aux,
                                        ...
                                        real* wave_speeds)
{
  ...
  for(row = num_ghost; row <= ny + num_ghost; ++row) {
    for(col = num_ghost; col <= nx + num_ghost; ++col) {
      ...
      advection_rp(q + idx_left*num_eqn, q + idx_center*num_eqn,
      ...
      advection_rp(q + idx_up*num_eqn, q + idx_center*num_eqn,
                   aux, aux,  &advection_rp_aux_global,
