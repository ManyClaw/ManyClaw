...
#pragma omp for schedule(runtime) nowait // shared(q, aux, amdq, apdq, wave, wave_speeds) private(col, row, idx_left, idx_center, idx_up, idx_out_x, idx_out_y)
  for(int idx=0; idx < (ny + num_ghost)*(nx+num_ghost); ++idx){
    row = idx / (nx + num_ghost);
    col = idx % (nx + num_ghost);
